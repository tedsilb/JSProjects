...
const main = () => {
    const btnResetGame = getEl("reset-button");
    const lblStatusDisplay = getEl("status-display");
    const cellA1 = getEl("A1");
    const cellA2 = getEl("A2");
    const cellA3 = getEl("A3");
    const cellB1 = getEl("B1");
    const cellB2 = getEl("B2");
    const cellB3 = getEl("B3");
    const cellC1 = getEl("C1");
    const cellC2 = getEl("C2");
    const cellC3 = getEl("C3");
    let cpuChoice;
    let cpuStarted;
    let buttonsEnabled;
    let availableCells = [];
    ...
    const takeCpuTurn = () => {
        ...
        if (isTied()) {
        ...
        else if (arraysEqual(userChosenCells, ["B2", "C3"]) &&
        ...
        else if ((includesAll(userChosenCells, ["A1", "A3"]) || includesAll(userChosenCells, ["B2", "C2"])) &&
        ...
        else if ((includesAll(userChosenCells, ["B2", "B3"]) || includesAll(userChosenCells, ["A1", "C1"])) &&
        ...
        else if ((includesAll(userChosenCells, ["B1", "B2"]) || includesAll(userChosenCells, ["A3", "C3"])) &&
        ...
        else if ((includesAll(userChosenCells, ["C1", "C3"]) || includesAll(userChosenCells, ["A2", "B2"])) &&
        ...
        else if ((includesAll(cpuChosenCells, ["A1", "A3"]) || includesAll(cpuChosenCells, ["B2", "C2"])) &&
            availableCells.includes("A2")) {
            cpuChoice = "A2";
        }
        ...
        else if ((includesAll(cpuChosenCells, ["B2", "B3"]) || includesAll(cpuChosenCells, ["A1", "C1"])) &&
            availableCells.includes("B1")) {
            cpuChoice = "B1";
        }
        ...
        else if ((includesAll(cpuChosenCells, ["B1", "B2"]) || includesAll(cpuChosenCells, ["A3", "C3"])) &&
            availableCells.includes("B3")) {
            cpuChoice = "B3";
            // C block
        }
        ...
        else if ((includesAll(cpuChosenCells, ["C1", "C3"]) || includesAll(cpuChosenCells, ["A2", "B2"])) &&
            availableCells.includes("C2")) {
            cpuChoice = "C2";
        }
            ...
            if (userChosenCells[0] === "B2") {
                cpuChoice = ["A1", "A3", "C1", "C3"][Math.floor(Math.random() * 4)];
            }
            else {
                cpuChoice = "B2";
            }
            ...
            else if (randomNo > 0.4 && randomNo <= 0.8) {
                itemIndex = 2;
            }
            else if (randomNo > 0.8 && randomNo <= 0.9) {
                itemIndex = 3;
            }
            else if (randomNo > 0.9 && randomNo <= 0.925) {
                itemIndex = 4;
            }
            else if (randomNo > 0.925 && randomNo <= 0.95) {
                itemIndex = 5;
            }
            else if (randomNo > 0.95 && randomNo <= 0.975) {
                itemIndex = 6;
            }
            else {
                itemIndex = 7;
        ...
        const removeIndex = availableCells.indexOf(cpuChoice);
    ...
    const buttonPress = (button, gridLoc) => {
        ...
        if (buttonsEnabled) {
            ...
            if (button.innerHTML === "") {
                ...
                const cellIndex = availableCells.indexOf(gridLoc);
